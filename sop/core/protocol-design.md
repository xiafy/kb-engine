...
IF open-label（含 ICC 对照）→ PFS 主要终点通常需 BICR；investigator PFS 为敏感性分析。
IF double-blind → PFS 可用研究者评估为主要终点；BICR 作为敏感性分析。
→ 加速审批终点见 `regulatory/accelerated-approval.md`
→ 肿瘤终点通用规则见 `domains/oncology-solid.md`

### 4.2 肿瘤终点层级
OS > PFS > ORR。传统审批：OS 或 PFS（有临床意义的改善）。
PFS 需满足：(1) magnitude 足够大，(2) BICR 确认（开放标签），(3) 亚组一致性，(4) ORR 等终点支持。
